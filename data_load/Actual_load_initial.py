...
def nys_realtime_data(target_month: str) -> pd.DataFrame:
    PAL_BASE_URL = f"https://mis.nyiso.com/public/csv/pal/{target_month}01pal_csv.zip"
    response = requests.get(PAL_BASE_URL)
    ...
    if not frames:
        raise ValueError("NYISO pal did not contain readable data.")
